...
- **Tagging applicativo**: il campo opzionale `job_tag` è accettato in `POST /ansible/playbook`, viene salvato nei metadati e permette di filtrare l'elenco dei job con `GET /ansible/jobs?tag=<job_tag>`, senza essere passato ai playbook. 【F:src/ansible_link.py†L139-L158】【F:src/ansible_link.py†L345-L368】
